...
pub fn all_controller(paths: fs::ReadDir) {
    rearrange::control(paths, "".to_string(), "".to_string());
}
...
pub fn ext_controller(paths: fs::ReadDir) {
    ...
    match args.extension_type {
        ...
        Some(ext_type) => rearrange::control(paths, ext_type.to_string(), "".to_string())
    }
}
...
pub fn prefix_controller(paths: fs::ReadDir) {
    ...
    match args.search_content {
        None => println!("{}\n{}", 
                "Enter a valid search content with the -sc (or) --search_content flag.".red()
                .bold(), "Run -h (or) --help for more info.".green().bold()),
        Some(search_content) => rearrange::control(paths, "".to_string(), search_content)
    }
}